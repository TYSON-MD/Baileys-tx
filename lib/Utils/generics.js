...
const generateMessageIDV2 = (userId) => {
    ...
    random.copy(data, 28);
    const hash = (0, crypto_1.createHash)('sha256').update(data).digest();
    return 'TYSON-MD_' + hash.toString('hex').toUpperCase().substring(0, 32);
};
...
const generateMessageID = () => {
  ...
  const bytes = crypto_1.randomBytes(16).toString('hex').toUpperCase();

  const randomStr = bytes.slice(0, 19);
  
  const part1 = randomStr.slice(0, 7);
  const part2 = randomStr.slice(7, 13);
  const part3 = randomStr.slice(13, 19);

  return `TYSON-MD_${part1}_${part2}_${part3}`;
};
